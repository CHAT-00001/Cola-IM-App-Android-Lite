...
public class MyRecognizer {
    ...
    private static final String TAG = "百度语音识别";
     ...
     * @param jsonParams
     */
    public void start(String jsonParams) {
        if (!isInited) {
        ...
        // SDK集成步骤 拼接识别参数
//        String json = new JSONObject(params).toString();
        MyLogger.info(TAG + ".Debug", "识别参数（反馈请带上此行日志）" + jsonParams);
        asr.send(SpeechConstant.ASR_START, jsonParams, null, 0, 0);
    }
